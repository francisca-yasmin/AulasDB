...
SELECT atividades.descricao, turmas.nome as turma, materias.nome as materia, instrutores.nome as instutor
FROM atividades
JOIN turmas ON atividades.turma_id = turmas.id
JOIN materias ON atividades.materia_id = materias.id
JOIN instrutores ON  atividades.instrutor_id = instrutores.id;

-- mostrar materias e instrutores
SELECT materias.nome as nome, instrutores.nome as nome
FROM materias
JOIN instrutor_materia ON instrutor_materia.materia_id = materias.id
JOIN instrutores ON instrutores.id = instrutor_materia.instrutor_id;

-- descricao de cada atividade, com o nome e especialidade do instutor
SELECT atividades.descricao as descricao, instrutores.nome, instrutores.especialidade
FROM atividades
JOIN instrutores ON instrutores.id = atividades.instrutor_id;

-- descrição de cada atividade, a data de início e data de término da turma e o nome da matéria 
SELECT  atividades.descricao as descricao, turmas.data_inicio, turmas.data_fim, materias.nome as materias
FROM atividades
JOIN turmas ON turmas.id = atividades.turma_id
JOIN materias ON atividades.materia_id  = materias.id;

--  nome do aluno, o nome da turma, a descrição da atividade realizada pela turma, o nome da matéria da atividade e o nome do instrutor responsável.
SELECT 
...
FROM atividades
JOIN turmas ON turmas.id = atividades.turma_id
JOIN alunos ON alunos.turma_id = turmas.id
JOIN materias ON atividades.materia_id = materias.id
JOIN instrutores ON instrutores.id = atividades.instrutor_id